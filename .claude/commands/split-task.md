...
1. Read `.claude/skills/issue-conventions/SKILL.md`, then `task.md` and `labels.md` in the same directory.
2. Fetch the issue:
   ...
   Note its parent, its labels, its relationships, and every decision its body already records. If it is not a Task, say so and stop.
...
6. Check every drafted body for questions, unmade decisions, relationships written as text, and file lists. Show the full set with labels, together with what will happen to the original, and ask for confirmation before creating anything.
